        ...
        <div className="flex items-center gap-2 flex-wrap text-[11px]">
          {(["Accepted", "Proposed", "Superseded", "Deprecated", "Unknown"] as const)
            ...
            .map((s) => (
              ...
              >
                {s}
                <span className="text-[10.5px] opacity-80 tabular-nums">{counts[s]}</span>
              </span>
            ))}
      ...
      {adrs.length === 0 ? (
        <div className="bg-white border border-dashed border-zinc-200 rounded-2xl px-6 py-12 text-center">
          ...
          <p className="mt-1 text-[11.5px] text-zinc-500">
            ...
            <code className="bg-zinc-100 rounded px-1 py-0.5 text-[10.5px]">
              docs/architecture/decisions/
            </code>{" "}
        ...
        <ol className="bg-white border border-zinc-200 rounded-2xl divide-y divide-zinc-100 overflow-hidden">
          {adrs.map((adr) => (
            <li key={adr.slug}>
              ...
              >
                <span className="size-7 rounded-lg bg-zinc-900 text-white text-[10.5px] font-mono font-semibold inline-flex items-center justify-center shrink-0 mt-0.5">
                  {String(adr.number).padStart(3, "0")}
                </span>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="text-[13.5px] font-semibold text-zinc-900">{adr.title}</h3>
                    <span
                  ...
                  {adr.summary && (
                    <p className="mt-1 text-[12px] text-zinc-600 leading-relaxed line-clamp-2">
                      {adr.summary}
                    </p>
                  )}
                  <div className="mt-2 flex items-center gap-3 text-[10.5px] text-zinc-400">
                    <code className="bg-zinc-50 rounded px-1.5 py-0.5">{adr.slug}.md</code>
                ...
                <ChevronRight className="size-3.5 text-zinc-300 mt-2 shrink-0" strokeWidth={2} />
              </Link>
            </li>
          ))}
        </ol>
