    ...
    <div className="mx-auto max-w-5xl px-6 py-6 space-y-5">
      <header>
        ...
        <p className="mt-1 text-[12.5px] text-suite-ink-3 max-w-2xl">
          ...
          <code className="text-[11px] bg-suite-card-soft px-1 py-0.5 rounded suite-num">
          ...
          {history.length > 1 && (
            <section className="bg-white border border-suite-line rounded-2xl p-4">
              <header className="mb-3">
                <h2 className="text-[12.5px] font-semibold text-suite-ink">Pass-rate trend</h2>
                <p className="text-[11px] text-suite-ink-3">
                  Last {history.length} runs from{" "}
                  <code className="text-[10.5px] bg-suite-card-soft px-1 py-0.5 rounded suite-num">
          ...
          <section>
            <header className="flex items-baseline justify-between mb-2.5 px-1">
              <h2 className="text-[12.5px] font-semibold text-suite-ink">
                Suites <span className="text-suite-ink-3 font-normal">· {run.suites.length}</span>
              </h2>
              <span className="text-[11px] text-suite-ink-3">Failed suites auto-expanded</span>
            </header>
...
function EmptyState() {
  return (
    <div className="bg-white border border-dashed border-suite-line rounded-2xl px-6 py-12 text-center">
      ...
      <h3 className="mt-3 text-[13px] font-semibold text-suite-ink">No JUnit report yet</h3>
      ...
      <p className="mt-3 text-[10.5px] text-suite-ink-4">
        Snapshot to <code className="bg-suite-card-soft rounded px-1 py-0.5 suite-num">tests/.junit-history/</code> with a sortable timestamp prefix to build the trend chart.
      </p>
    </div>
  );
}
