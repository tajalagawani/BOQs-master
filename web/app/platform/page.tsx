  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-6 space-y-6">
      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <div className="text-[11px] uppercase tracking-[0.12em] text-zinc-500 font-medium">
            Welcome back, {user.name.split(" ")[0]}
          </div>
          <h1 className="mt-1 text-[clamp(24px,2.4vw,32px)] leading-tight font-semibold tracking-tight text-zinc-900">
        ...
        >
          ...
          <ExternalLink className="size-3 text-zinc-400" strokeWidth={1.75} />
        </a>
      </header>
      ...
      <section className="bg-white border border-zinc-200 rounded-2xl p-5">
        <header className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-sm font-semibold text-zinc-900">Sign-off framework — components</h2>
          ...
          <Link
            href="/platform/kpis"
            className="text-[12px] font-medium text-zinc-700 hover:text-zinc-900 inline-flex items-center gap-1"
          >
          ...
          {summary.byComponent.map((c) => {
            ...
            return (
              ...
              >
                ...
                <div className="mt-0.5 text-[13px] font-semibold text-zinc-900 line-clamp-2 min-h-[2.6em]">
                  {c.component}
                </div>
                <div className="mt-2 text-[20px] font-bold tabular-nums text-zinc-900">
                  {c.green}
                  <span className="text-[13px] font-normal text-zinc-500"> / {c.total}</span>
                </div>
                <div className="mt-2 h-1.5 rounded-full bg-zinc-200 overflow-hidden">
                  <div className="h-full bg-emerald-500" style={{ width: `${pct}%` }} />
                </div>
                <div className="mt-1 text-[11px] text-zinc-500">{pct}% met</div>
              </div>
            );
          })}
        </div>
      ...
      <section className="bg-white border border-zinc-200 rounded-2xl p-5">
        <header className="flex items-center justify-between mb-3">
          <div>
            <h2 className="text-sm font-semibold text-zinc-900">Recently updated docs</h2>
            <p className="text-[11px] text-zinc-500">Last 5 modifications to the docs/ tree.</p>
          </div>
          <Link
            href="/platform/docs"
            className="text-[12px] font-medium text-zinc-700 hover:text-zinc-900 inline-flex items-center gap-1"
        ...
        <ul className="divide-y divide-zinc-100">
          {recentDocs.map((d) => (
            <li key={d.slug} className="py-2 flex items-center justify-between gap-3">
              ...
              >
                <BookOpen className="size-3.5 text-zinc-400 shrink-0" strokeWidth={1.75} />
                <span className="truncate font-medium">{d.label}</span>
                <span className="text-[11px] text-zinc-400 truncate hidden sm:inline">/{d.slug}</span>
              </Link>
